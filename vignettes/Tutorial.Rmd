...
When sufficient data are available, the function `index calculation` can be used to calculate the temporal development of the catchment connectivity based on changing habitat weights and dams passabilities. The function `t_index_calculation` does this job, since manually updating sequentially the passabilities and the habitat weights can be tedious. Two functions (`t_passability_sequencer` and `t_weights_sequencer`) are provided to properly format the passability and weights data to input to `t_index_calculation`.
