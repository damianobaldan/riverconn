...
This vignette is meant to: (a) give a short theoretical background of equations and methods implemented in riverconn, and (b) provide an example of application of the main riverconn functionalities on a simple network (15 nodes, to keep low the computational load). For a real-world example (Ebro river, NW Spain, ca. 600 nodes), check [this](https://damianobaldan.github.io/riverconn_tutorial/) tutorial.
...
#### Dispersal probability definition
The dispersal probability depends on several factors: the presence of barriers between nodes $i$ and $j$ , the presence of suitable habitats in nodes $i$ and $j$, the distance between $i$ and $j$, and the quality of the habitat alongside the connection. The value of $I_{ij}$ is thus determined by several contributions. The product of such contributions is used to follow a 'one-bad-all-bad' approach:

$$
\ I_{ij} = c_{ij}B_{ij}
$$
where $c_{ij}$ is a 'barrier dispersal probability' depending on the presence of barriers between nodes $i$ and $j$, and  $B_{ij}$ is a 'dispersal dispersal probability' depending on the distance between nodes $i$ and $j$.

##### Barrier dispersal probability 
...
##### Species dispersal probability.

The species dispersal probability can be calculated as a function of the distance between reaches. An exponential dispersal kernel can be used:
...
Here we add the barriers information as edge attribute, including the field 'id_barrier', and the reach information data as vertices attributes, including the length and the corresponding habitat suitability index.

```{r g decorate, message = FALSE, collapse = TRUE, width = 60, warning = FALSE}
# Decorate edges 
E(g)$id_barrier <- c("1", "NA", "2", "3", "NA", "4", "NA", "5", "6", "NA",  "NA", "NA", "NA", "7", "NA")
E(g)$type <- ifelse(is.na(E(g)$id_barrier), "joint", "barrier")
E(g)
...
g_e_df <- igraph::as_data_frame(g, what = "edges") %>%
  mutate(pass_u = ifelse(!is.na(id_barrier),0.1,NA),
         pass_d = ifelse(!is.na(id_barrier),0.7,NA))
g_e_df
...
The function `d_index_calcualtion` allows to calculate the CCI and RCI changes when barriers are removed. Metadata on which barriers are to be removed and how the passability changes are provided in the 'barriers_metadata' data.frame. Parallel calculations can be activated if the network is too large (thousands of vertices and edges).

```{r ibarriers metadata, message = FALSE, collapse = TRUE, width = 60, warning = FALSE}
barriers_metadata <- data.frame("id_barrier" =  as.character(1:7),
...
g_v_df
g_e_df <- igraph::as_data_frame(g, what = "edges") %>%
  left_join(d_index, by = "id_barrier")
g_e_df
...
When sufficient data are available, the function `index_calculation` can be used to calculate the temporal development of the catchment connectivity based on changing habitat weights and barriers passabilities. The function `t_index_calculation` does this job, since sequential manual update of the passabilities and habitat weights can be tedious. Two functions (`t_passability_sequencer` and `t_weights_sequencer`) are provided to properly format the passability and weights data to input to `t_index_calculation`.
...
barriers_metadata <- t_passability_sequencer(barriers_metadata, seq_ops)
head(barriers_metadata)
